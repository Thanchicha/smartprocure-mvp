...
function verifySpecsMatch(orderedKg, actualKg, tolerance = 0.1) {
  ...
  console.log(
    `[HARDWARE-SYNC] Specs check — Ordered: ${orderedKg}kg | Actual: ${actualKg}kg | Deviation: ${(deviation * 100).toFixed(1)}%`,
  );
...
function outputStatusSuccess(data) {
  console.log(
    "[HARDWARE-SYNC] ✅ OUTPUT: Status SUCCESS — Stock accepted. Updating central DB...",
  );
...
function outputStatusWarning(data) {
  console.log(
    "[HARDWARE-SYNC] ⚠️ OUTPUT: Status WARNING — Weight mismatch! Alert sent to staff.",
  );
  console.log("[HARDWARE-SYNC] Data:", data);
...
function initHardwareWebSocket() {
  console.log(
    "[HARDWARE-SYNC] initHardwareWebSocket() — TODO: Connect to wss://iot.pakarang-supply.com/scanner",
  );

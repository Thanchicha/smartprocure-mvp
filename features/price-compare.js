...
async function fetchMakroPrice(ingredientName, source = "makro") {
  console.log(
    `[PRICE-COMPARE] Fetching market price for: "${ingredientName}" from source: ${source}`,
  );
...
async function compareAllPrices(items) {
  console.log(
    "[PRICE-COMPARE] Starting full price comparison for",
    items.length,
    "items...",
  );
...
function renderCostSavingDisplay(comparisons) {
  console.log(
    "[PRICE-COMPARE] renderCostSavingDisplay() — TODO: Render comparison UI to #cost-saving-section",
  );

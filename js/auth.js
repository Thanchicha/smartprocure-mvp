...
function isLoggedIn() {
  try {
    return JSON.parse(localStorage.getItem(AUTH_KEY))?.ok === true;
  } catch {
    return false;
  }
}
function setSession() {
  localStorage.setItem(AUTH_KEY, JSON.stringify({ ok: true }));
}
...
function handleLogin(e) {
  ...
  setTimeout(() => {
    if (user === CREDS.username && pass === CREDS.password) {
      ...
      btn.textContent =
        typeof t === "function" ? t("login.submit") : "เข้าสู่ระบบ";
    }
  }, 550);
}

/**
 * ui-render.js — จัดการแสดงผลข้อมูลลงบนหน้าจอ (DOM Manipulation)
 */
  ...
  container.innerHTML = MEAL_KEYS.map(meal => {
    ...
    return `<div class="meal-cfg-card">
      ...
      <div class="field-group">
        ...
        <input type="number" class="field-input" min="0" max="100" value="${ms.mealRate}" onchange="saveMealCfg('${meal}','mealRate',this.value)" />
      </div>
      <div class="field-group" style="margin-top:0.5rem">
        <label class="field-label">BufferFactor (× เผื่อบุฟเฟต์)</label>
        <input type="number" class="field-input" min="1" step="0.05" value="${ms.bufferFactor}" onchange="saveMealCfg('${meal}','bufferFactor',this.value)" />
      </div>
    </div>`;
  }).join('');
}
...
function renderDaily(totalRooms, occupancyRate, guestsPerRoom, bufferRate, cfg) {
  ...
  if (dailyEl) {
    dailyEl.innerHTML = Object.values(meals).map(m => `
...
function showToast(msg) {
  ...
  if (!toast) {
    ...
    toast.id = 'sp-toast';
    toast.style.cssText = `position:fixed;bottom:1.5rem;right:1.5rem;z-index:1000;background:var(--navy);color:#fff;font-family:var(--font);font-size:0.875rem;padding:0.75rem 1.25rem;border-radius:0.625rem;box-shadow:0 4px 20px rgba(0,0,0,0.2);opacity:0;transition:opacity 0.25s;pointer-events:none;`;
    document.body.appendChild(toast);
  }
  toast.textContent = msg; toast.style.opacity = '1'; clearTimeout(toast._timer);
  toast._timer = setTimeout(() => { toast.style.opacity = '0'; }, 3000);
}

function showDupModal(item) {
  const mealRows = MEAL_KEYS.filter(m => isMealActive(item, m)).map(m => `<div class="dup-row"><span class="dup-label">${MEAL_META[m].icon} ${MEAL_META[m].label}</span><span class="dup-val">${getGrams(item, m)} กรัม/คน</span></div>`).join('');
  const info = document.getElementById('dup-info');
  if (info) info.innerHTML = `<div class="dup-row"><span class="dup-label">ชื่อ</span><span class="dup-val bold">${esc(item.name)}</span></div><div class="dup-row"><span class="dup-label">หมวดหมู่</span><span class="dup-val">${esc(item.category)}</span></div><div class="dup-row"><span class="dup-label">ราคา/กก.</span><span class="dup-val">${item.pricePerKg.toLocaleString()} ฿</span></div>${mealRows ? `<div style="margin-top:0.5rem;font-size:0.7rem;color:var(--muted);text-transform:uppercase;letter-spacing:0.05em;font-weight:600;padding:0.25rem 0">มื้อที่ใช้</div>${mealRows}` : ''}`;
  const editBtn = document.getElementById('dup-edit-btn');
  if (editBtn) editBtn.onclick = () => openEditModalFromDuplicate(item.id);
...
function openEditModalFromDuplicate(id) { document.getElementById('dup-modal')?.classList.add('hidden'); openEditModal(id); }

function openEditModal(id) {
  const item = getAllItems().find(i => i.id === id);
  if (!item) return;
  editingItemId = id;
  setText('em-name', item.name); setText('em-cat', item.category); setText('em-price', item.pricePerKg.toLocaleString() + ' ฿/กก.');
  MEAL_KEYS.forEach(meal => {
    const cb = document.getElementById(`em-cb-${meal}`), inp = document.getElementById(`em-g-${meal}`), row = document.getElementById(`em-grams-row-${meal}`);
    const active = isMealActive(item, meal), grams = getGrams(item, meal);
    if (cb) cb.checked = active; if (inp) inp.value = grams; if (row) row.style.display = active ? 'flex' : 'none';
  });
  updateEditCalc(item); document.getElementById('edit-modal')?.classList.remove('hidden');
}

function closeEditModal(e) {
  if (e && e.target !== document.getElementById('edit-modal')) return;
  document.getElementById('edit-modal')?.classList.add('hidden'); editingItemId = null;
}

function toggleEditMealRow(meal) {
  const cb = document.getElementById(`em-cb-${meal}`), row = document.getElementById(`em-grams-row-${meal}`);
...
function updateEditCalc(item) {
  ...
  const cb = document.getElementById(`em-cb-${activeMeal}`), inp = document.getElementById(`em-g-${activeMeal}`), grams = cb?.checked ? (parseFloat(inp?.value) || 0) : 0;
  const baseKg = (grams * mg * ms.bufferFactor) / 1000, recommendedKg = baseKg * (1 + bufferRate / 100), cost = recommendedKg * item.pricePerKg;
  setText('em-base', baseKg.toFixed(2) + ' กก.'); setText('em-rec', recommendedKg.toFixed(2) + ' กก.'); setText('em-cost', cost.toLocaleString('th-TH',{maximumFractionDigits:0}) + ' ฿');
  const recLabel = document.getElementById('em-rec-label'); if (recLabel) recLabel.textContent = `แนะนำสั่ง (+${bufferRate}%)`;
}

function toggleNewGrams(meal) {
  const cb = document.getElementById(`new-cb-${meal}`), row = document.getElementById(`new-grams-row-${meal}`);
...
function initAutocomplete() {
  ...
  if (!inp) return;
  const wrap = document.createElement('div'); wrap.className = 'autocomplete-wrap'; inp.parentNode.insertBefore(wrap, inp); wrap.appendChild(inp);
  const drop = document.createElement('div'); drop.className = 'autocomplete-drop'; drop.id = 'ac-drop'; wrap.appendChild(drop);
  function openDrop(items, query) {
    if (!query) { closeDrop(); return; }
    const q = query.toLowerCase(), matches = items.filter(i => i.name.toLowerCase().includes(q)).slice(0,5);
    if (matches.length === 0) drop.innerHTML = `<div class="autocomplete-empty">ไม่พบวัตถุดิบ — กรอกข้อมูลเองได้เลย</div>`;
    else {
      drop.innerHTML = matches.map(item => {
        const idx = item.name.toLowerCase().indexOf(q), pre=esc(item.name.slice(0,idx)), hi=esc(item.name.slice(idx,idx+query.length)), post=esc(item.name.slice(idx+query.length));
      ...
      drop.querySelectorAll('.autocomplete-item').forEach(el => {
        el.addEventListener('mousedown', (e) => {
          e.preventDefault(); const item = items.find(i => i.id === el.dataset.id); if (!item) return;
          inp.value = item.name;
          const catSel = document.getElementById('new-cat'), priceInp = document.getElementById('new-price');
          if (catSel) catSel.value = item.category; if (priceInp) priceInp.value = item.pricePerKg;
          MEAL_KEYS.forEach(meal => {
            const cb = document.getElementById(`new-cb-${meal}`), gr = document.getElementById(`new-g-${meal}`), row = gr?.closest('.new-grams-row');
            if (cb && item.meals?.[meal]) { cb.checked = item.meals[meal].active; if (gr) gr.value = item.meals[meal].grams; if (row) row.classList.toggle('hidden', !cb.checked); }
          });
          closeDrop();
        });
      });
